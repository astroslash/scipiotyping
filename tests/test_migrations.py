...
def test_v1_database_migrates_without_data_loss(tmp_path):
    ...
    connection.execute("INSERT INTO profiles(name,created_at) VALUES('Emily','2026-01-02')")
    connection.commit(); migrate(connection)
    assert [row[0] for row in connection.execute("SELECT name FROM profiles ORDER BY id")] == ["Kenneth", "Emily"]
    ...
    levels = dict(connection.execute("SELECT name,school_level FROM profiles"))
    assert levels == {"Kenneth": "middle", "Emily": "elementary"}
    assert "school_level" in {row[1] for row in connection.execute("PRAGMA table_info(custom_passages)")}
    connection.close()
